...
exports.login = (req, res, next) => {
    User.findOne({ email: req.body.email }) //recherche si email existant dans la bdd
        .then((user) => {
            ...
            bcrypt.compare(req.body.password, user.password) //compare le mdp saisi avec celui de la bdd
                .then((valid) => {
                    if (!valid) {
                        return res.status(401).json({ message: "Login ou mot de passe incorrect" });
                    }
                    res.status(200).json({
                        userId: user._id,
                        token: jwt.sign( //crypte userId pour délivrer un token temporaire
                            { userId: user._id },
