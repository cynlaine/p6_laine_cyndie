...
exports.createSauce = (req, res, next) => {
    const sauceObject = JSON.parse(req.body.sauce);  //parse la requête form-data en json
...
exports.updateSauce = (req, res, next) => {
    const sauceObject = req.file ? { //si requête contient une image on parse le form-data en json
...
exports.likeSauce = (req, res, next) => {
    ...
    switch (like) {
        ...
        case 0 : //si l'user annule son like/dislike
            Sauce.findOne({ _id: req.params.id })
            .then((sauce) => {
                if (sauce.usersLiked.includes(req.body.userId)) { //si le tableau usersLiked contient l'ID de l'user on le supprime
                    Sauce.updateOne(
                        { _id: req.params.id },
                    ...
                    .catch((error) => res.status(400).json({ error }));
                } else if (sauce.usersDisliked.includes(req.body.userId)) { //si le tableau usersDisliked contient l'ID de l'user on le supprime
                    Sauce.updateOne(
                        { _id: req.params.id },
...
// fonction like / dislike
// exports.likeSauce = (req, res, next) => {
//     if (req.body.like === 1) { // si l'user a liké la sauce on ajoute son ID au tableau usersLiked
//         Sauce.updateOne(
//             { _id: req.params.id },
//             { $push: { usersLiked: req.body.userId }, $inc: { likes: 1 }}
//         )
//         .then(() => res.status(200).json({ message: "Sauce likée" }))
//         .catch((error) => res.status(400).json({ error }));
//     } else if (req.body.like === -1) { //si l'user a disliké la sauce on ajoute son ID au tableau usersDisliked
//         Sauce.updateOne(
//             { _id: req.params.id },
//             { $push: { usersDisliked: req.body.userId }, $inc: { dislikes: 1 }}
//         )
//         .then(() => res.status(200).json({ message: "Sauce dislikée" }))
//         .catch((error) => res.status(400).json({ error }));
//     } else if (req.body.like === 0) { //si l'user supprime son like/dislike, on supprime son ID du tableau correspondant
//         Sauce.findOne({ _id: req.params.id })
//         .then((sauce) => {
